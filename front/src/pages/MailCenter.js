...
import SelectItems,{UserItemShow} from '../components/Select'
		...
		.then(result => {
			if (result.result=='ok'){
				...
				for(var i in result.data){
					result.data[i].pubdate=result.data[i].pubdate*1000
				}
...
function SendMail(){
	...
	function finishedSelect(selectedItems){
	...
	function submitForm(data){
		...
		for(var i in receivers){
			data.receivers.push(receivers[i])
		}
		//发送请求
